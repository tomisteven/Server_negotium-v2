    ...
    client.nextServices.forEach((service) => {
      services.push({
        title:
          ...
          client.apellido +
          " - " +
          service.fecha.getHours() +
          ":" +
          service.fecha.getMinutes() +
          `${service.fecha.getHours() > 12 ? " pm" : " am"}`,
        date: service.fecha,
      });
    });
...
const deleteDeuda = async (req, res) => {
  ...
  if (response.deudas < 0) response.deudas = 0;
...
const createClient = async (req, res) => {
  ...
  if (response.clientes.length == membresia_active.clientes_max) {
    return res
      .status(400)
      .json({ message: "No se puede agregar más clientes", ok: false });
  } else {
    ...
    if (client) {
      ...
      res
        .status(200)
        .json({ message: "Cliente creado", client: req.body, ok: true });
    }
  }
};
...
const getClientConDeuda = async (req, res) => {
  ...
  const clientsConDeuda = response.clientes.filter(
    (client) => client.deuda == true
  );
...
const clientesConDeudaItem = async (req, res) => {
  ...
  const response = await User.findById(user_id);
  if (!response) res.status(404).json({ message: "No hay cliente con ese id" });
  const clientsItem = response.clientes.filter(
    (client) => client.deuda === false
  );
  const items = [];
...
const getClientSinDeuda = async (req, res) => {
  ...
  const response = await User.findById(user_id);
  if (!response) res.status(404).json({ message: "No hay cliente con ese id" });
  const clientSinDeuda = response.clientes.filter(
    (client) => client.deuda == false
  );
  if (clientSinDeuda != null && clientSinDeuda.length > 0)
    res.status(200).json(clientSinDeuda);
...
const completeServiceFuture = async (req, res) => {
  ...
  const service = client.nextServices.find(
  ...
  res.status(200).json({
    ...
    client: client.serviciosadquiridos,
  });
};
...
const addServiceFuture = async (req, res) => {
  ...
  const service = response.servicios.find(
    (service) => service.nombre == req.body.nombre
  );
...
const updateClient = async (req, res) => {
  ...
  if (client != null) {
    ...
    client.gastoTotal == 0
      ? (client.gastoTotal = req.body.gastoTotal)
      : (client.gastoTotal = req.body.gastoTotal);
    client.serviciosadquiridos =
      req.body.serviciosadquiridos || client.serviciosadquiridos;
...
const deleteServiceClient = async (req, res) => {
  ...
  const client = response.clientes.find((client) => client._id == client_id);
  if (client) {
    const service = client.serviciosadquiridos.find(
      (service) => service._id == service_id
    );
    if (service != null) {
      client.serviciosadquiridos.remove(service);
      client.gastoTotal -= service.precio;
      if (client.gastoTotal < 0) client.gastoTotal = 0;
...
const loginClient = async (req, res) => {
  ...
  const client = response.clientes.find(
    (client) =>
      (client.email == username && client.password == password) ||
...
const updateUsernamePassword = async (req, res) => {
  ...
  if (client != null) {
    ...
    const us = response.clientes.find(
      (client) => client.username == req.body.username
    );
    if (us != null)
      res
        .status(404)
        .json({ message: "Ya existe un cliente con ese username" });
    else {
